...
describe('calculateDailyGoals', () => {
  describe('BMR számítás', () => {
    ...
    it('helyes BMR-t számol női felhasználónak', () => {
      ...
      const result = calculateDailyGoals(30, 'female', 165, 60, 1, 2)
    ...
    it('helyes BMR-t számol egyéb nemű felhasználónak', () => {
      ...
      const result = calculateDailyGoals(25, 'other', 170, 70, 1, 2)
  ...
  describe('Cél alapján kalória módosítás', () => {
    it('fogyásnál 500 kalóriát levon', () => {
      // goalId = 1 -> fogyás
      const result = calculateDailyGoals(25, 'male', 180, 80, 1, 1)
    ...
    it('hízásnál 500 kalóriát hozzáad', () => {
      // goalId = 3 -> hízás
      const result = calculateDailyGoals(25, 'male', 180, 80, 1, 3)
  ...
  describe('Makró számítások', () => {
    ...
    it('aktívabb életmódnál magasabb fehérje cél', () => {
      const sedentary = calculateDailyGoals(25, 'male', 180, 80, 1, 2) // Ülő
      const active = calculateDailyGoals(25, 'male', 180, 80, 4, 2) // Nagyon aktív
      expect(active.proteinGoal).toBeGreaterThan(sedentary.proteinGoal)
    })

    it('fogyásnál magasabb fehérje szorzó', () => {
      const maintenance = calculateDailyGoals(25, 'male', 180, 80, 1, 2) // Súlytartás
      const weightLoss = calculateDailyGoals(25, 'male', 180, 80, 1, 1) // Fogyás
      expect(weightLoss.proteinGoal).toBeGreaterThan(maintenance.proteinGoal)
    })

    it('hízásnál magasabb fehérje szorzó', () => {
      const maintenance = calculateDailyGoals(25, 'male', 180, 80, 1, 2) // Súlytartás
      const weightGain = calculateDailyGoals(25, 'male', 180, 80, 1, 3) // Hízás
  ...
  describe('Realisztikus esetek', () => {
    it('20 éves aktív férfi - izomépítés', () => {
      const result = calculateDailyGoals(20, 'male', 185, 85, 4, 3)
    ...
    it('50 éves ülő életmódú nő - fogyás', () => {
      const result = calculateDailyGoals(50, 'female', 160, 75, 1, 1)
